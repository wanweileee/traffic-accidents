...
model = ChebNet(in_channels=x.size(1), hidden_channels=16, out_channels=1, K=3).to(device)
data = data.to(device)
...
plt.title("Confusion Matrix (ChebNet)")
plt.colorbar()
...
plt.ylabel('True')
plt.tight_layout()
plt.savefig('confusion_matrix_chebnet.png')
plt.close()
...
plt.title(f"Feature Importance for Node {node_idx} (ChebNet)")
for i, bar in enumerate(bars):
    plt.text(bar.get_width() + 0.01, bar.get_y() + 0.3, f"{sorted_importance[i]:.3f}")
plt.tight_layout()
plt.savefig('feature_importance_chebnet.png')
plt.close()